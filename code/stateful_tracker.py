"""Shift-level stateful tracking for operator fatigue and equipment defect recurrence.

Why this exists
---------------
The baseline per-row auditor reads events in isolation. A per-row prompt cannot
see hazards that build over time across a shift:

  1. Cumulative fatigue: an operator logging multiple routine movements whose
     total elapsed time crosses the 12.0-hour statutory ceiling.
  2. Defect escalation: repeated minor hydraulic or mechanical warnings on the
     same asset (e.g. RS-42) that trigger mandatory red-tagging / removal from
     service under 29 CFR 1910.178(q)(7) rather than continuous operation.

This module maintains rolling state over the yard log timestamps so the auditor
can flag temporal infractions that single-row inspection misses.
...
class YardStateTracker:
    ...
    def process_event(self, row):
        ...
        recent_defects = [e for e in history if e["time"] >= cutoff and e["incident"] != "None"]

        if incident in ("Hydraulic Leak", "Load Imbalance", "Tire Pressure Warning"):
            ...
            self.equipment_history[equip].append({"time": t, "incident": incident, "log_id": log_id})
...
def run_stateful_audit():
    ...
    for entry in audit_log:
        for f in entry["findings"]:
            print(f"  [{entry['log_id']} @ {entry['timestamp']}] {f['citation']} ({f['type']}): {f['detail']}")
